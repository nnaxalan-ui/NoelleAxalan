...
SELECT 
    CASE
      WHEN `Previous_GPA` BETWEEN 3.6 AND 3.8 THEN 'A/B Border'
      WHEN `Previous_GPA` BETWEEN 2.6 AND 2.8 THEN 'B/C Border'
      WHEN `Previous_GPA` BETWEEN 1.6 AND 1.8 THEN 'C/D Border'
      ELSE 'Safe'
    END AS `GPA_Group`,
...
SELECT 
    CASE
      WHEN `Previous_GPA` >= 3.7 AND `Previous_GPA` <= 3.8 THEN 'At Risk A'
      WHEN `Previous_GPA` >= 3.6 AND `Previous_GPA` < 3.7 THEN 'Aspiring B'
      WHEN `Previous_GPA` >= 2.7 AND `Previous_GPA` <= 2.8 THEN 'At Risk B'
      WHEN `Previous_GPA` >= 2.6 AND `Previous_GPA` < 2.7 THEN 'Aspiring C'
      WHEN `Previous_GPA` >= 1.7 AND `Previous_GPA` <= 1.8 THEN 'At Risk C'
      WHEN `Previous_GPA` >= 1.6 AND `Previous_GPA` < 1.7 THEN 'Aspiring D'
      ELSE 'Safe'
    END AS `GPA_Status`,
    ROUND(AVG (`Exam_Anxiety_Score`), 2) AS `Avg_Anxiety`,
    STDDEV(`Exam_Anxiety_Score`) AS `Anxiety_Sigma`,
...
WITH `JeopardyStudents` AS (
  ...
  END AS `GPA_bucket`
  FROM `noelle-project-05-2026.Project_Data.student_performance_data`
  WHERE `Previous_GPA` BETWEEN 1.6 AND 1.8
)
